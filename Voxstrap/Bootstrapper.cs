...
using Voxstrap.AppData;
using Voxstrap.RobloxInterfaces;
using Voxstrap.UI.Elements.Bootstrapper.Base;
...
namespace Voxstrap
{
    ...
    {
        [DllImport("RuntimeFFlagEditor.dll", CallingConvention = CallingConvention.Cdecl, CharSet = CharSet.Ansi)]
        public static extern int InjectFlagsFromJSON(string jsonPath);
        ...
        public string BackgroundUpdaterMutexName => $"Voxstrap-BackgroundUpdater-{_launchMode}";

        public string MutexNamePrefix { get; set; } = "Voxstrap-Bootstrapper";
        ...
        private void StartRoblox()
        {
            ...
            try
            {
                ...
                string flagsPath = Path.Combine(Paths.Modifications, "ClientAppSettings.json");
                Task.Run(() => InjectFlagsFromJSON(flagsPath));
            }
        ...
        private async Task<bool> CheckForUpdates()
        {
            ...
            {
                App.Logger.WriteLine(LOG_IDENT, $"More than one Voxstrap instance running, aborting update check");
                return false;
            }
            ...
            SetStatus(Strings.Bootstrapper_Status_UpgradingVoxstrap);

            try
            {
#if DEBUG_UPDATER
                string downloadLocation = Path.Combine(Paths.TempUpdates, "Voxstrap.exe");
                ...
                catch (Exception ex)
                {
                    ...
                    if (ex.GetType() == typeof(ChecksumFailedException))
                    {
                        ...
                        Frontend.ShowConnectivityDialog(
                            ...
                            String.Format(Strings.Dialog_Connectivity_UnableToDownloadReason, "[https://Voxstraplabs.com/wiki/help/Voxstrap-cannot-download-roblox/](https://Voxstraplabs.com/wiki/help/Voxstrap-cannot-download-roblox/)"),
